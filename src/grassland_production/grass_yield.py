...
class Yield:
    ...
    def __init__(self,ef_country, calibration_year, target_year, scenario_data, scenario_animals_df,basline_animals_df):
        
        self.data_manager_class = DataManager(calibration_year, target_year, scenario_data, scenario_animals_df,basline_animals_df)
        self.fertiliser_class = Fertilisation(ef_country, calibration_year, target_year, scenario_data, scenario_animals_df,basline_animals_df)
    ...
    def get_yield(self):
        ...
        yield_per_ha = {farm_type: {sc: pd.DataFrame(0,
        ...
        for sc in scenario_list:
            ...
            for farm_type in keys:
                ...
                for (
                    grassland_type
                ) in fertilization_by_system_data_frame.index.levels[0]:

                    for soil_group in self.soil_class_yield_gap.keys():
                        yield_per_ha_df = yield_per_ha[farm_type][sc]
                        soil_class_prop = self.soil_class_prop[farm_type].loc[int(self.calibration_year), soil_group]
                        
                        yield_per_ha_df.loc[grassland_type, int(self.calibration_year)] += (
                            self._yield_response_function_to_fertilizer(
                                fert_rate[farm_type][sc].loc[grassland_type, str(self.calibration_year)],
                                grassland_type,
        ...
        transposed_yield_per_ha = {sc: {farm_type: yield_per_ha[farm_type][sc].T
                                    for farm_type in keys}
                                for sc in scenario_list}
